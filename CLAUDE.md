...
- **Round history** (`/rounds`): all rounds for logged-in user, gross + score vs par; cards are tappable → `/rounds/[id]`; **delete button** (trash icon) on each card — `window.confirm` prompt, deletes scores then round (FK order), removes from local state optimistically
