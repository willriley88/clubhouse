...
app/
  ...
  layout.tsx            # Root layout (NO BottomNav — splash screen bug); fetches ClubConfig and wraps children in ClubConfigProvider; PWA metadata
  manifest.ts           # Dynamic PWA manifest (Next.js convention) — reads ClubConfig, served at /manifest.webmanifest
  ...
  club/page.tsx         # Club tab — quick links (config-driven), tee sheet (Join), announcement feed
  ...
  components/
    ...
    ClubConfigProvider.tsx     # React context provider + useClubConfig() hook — fed by RootLayout
...
public/                 # PWA manifest now generated by app/manifest.ts (no static manifest.json)
...
supabase/migrations/
  ...
  20260407_chat_gin.sql              # SUPERSEDED by 20260409_chat.sql — skip
  ...
  20260409_gin.sql                   # gin_requests table (Guest in Need) — table created but no UI
                                     # currently consumes it; safe to skip until that feature lands
  20260410_schema_cleanup.sql        # updated_at, score_format, differential, indexes, soft-delete (run once)
  20260412_events_seed.sql           # real LeBaron events (Cinco De Mayo, SEAL, Mass Am, etc.)
  ...
  20260429_club_config_extensions.sql # club_config: external links + course meta + nav_links jsonb
...
- GPS page (`app/gps/page.tsx`) still has a hardcoded `HOLES` array of LeBaron coordinates instead of fetching from the `holes` table — drift from CLAUDE.md, fix when adding a second course
...
> Note: `20260409_gin.sql` creates a `gin_requests` table for a Guest-in-Need feature that has no UI yet — safe to skip for now; run it when that feature is built.

---

## Multi-Club Architecture
- `lib/club-config.ts` exports `ClubConfig` type + `getClubConfig(courseId?)` server helper.
- `app/layout.tsx` (server component) fetches the config once and wraps children in `<ClubConfigProvider>`.
- `app/components/ClubConfigProvider.tsx` exposes a `useClubConfig()` hook for client components.
